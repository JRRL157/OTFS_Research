...
simulation_params = [
    3, 3.5, 15;    % Low mobility, Sub-6 GHz
    60, 28, 120;   % Medium mobility, mmWave
    300, 6, 30;    % High-speed train, mid-band 5G
    27000, 20, 240 % Satellite LEO, Ka-band
];
    ...
    for mod_exp = 1:3
        ...
        for type = 1:4
          ...
          for idx = 1:length(SNR_values)
              SNR_db = SNR_values(idx); % Valor atual de SNR
              total_errors = 0;
              total_bits = 0;
              ...
              BER_values(type, idx) = total_errors / total_bits;
          end
        end
