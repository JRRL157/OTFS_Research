	...
	for (unsigned i = 0; i < N0.size(); i++) {
		...
		while ((n_blks_done < n_max_blks) && (n_blk_errs[i] < 100)) {
			...
			if (n_blks_done % 10 == 0 || n_blks_done == 1) {
				print_len = snprintf(str, 100, "Elapsed time: %.1f seconds, # tx blocks: %d,# error blocks:%d, ber: %.5f, bler %.5f", double(clock() - tStart) / CLOCKS_PER_SEC, n_blks_done, n_blk_errs[i], ber[i], bler[i]);
				cout << std::string(print_len, '\b') << str << flush;
			}
		}

		// print  progress when one SNR is finished
		snprintf(str, 100, "Elapsed time: %.1f seconds, # tx blocks: %d,# error blocks:%d, ber: %.5f, bler %.5f", static_cast<double>(clock() - tStart) / CLOCKS_PER_SEC, n_blks_done, n_blk_errs[i], ber[i], bler[i]);
		cout << std::string(print_len, '\b') << str << flush;
	}
